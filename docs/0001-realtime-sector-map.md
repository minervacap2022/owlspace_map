...
Regressions consistently trace to a dimension 2–7 factor being invisible at change time: a serialization-default 403 (Behavior); a bare-env launcher crash-loop (Context); two-fork drift (Boundaries); re-introduced fixed bugs (Intent/History); the missing feedback loop / untested paths (Change-safety + Tests & Coverage).
...
- **Multi-repo caveat:** the frontend and backend often live in separate repos; the map must index both.
