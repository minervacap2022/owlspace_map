# PRD — Klik No-New-Bugs Development System (codename **Sentinel**)

| | |
|---|---|
| **Status** | Draft v1.2 — 2026-06-01 |
| **Owner** | Wilson |
| **Related** | `ADR-0001` (sector-map design) · `/no-new-bugs` skill (the discipline) · KLIK 开发规范 wiki 规划中 011/012 · code-graph tool research (memory) |
...
Klik is built fast, by a human and by AI agents, across a Kotlin-Multiplatform frontend and a multi-service Python backend. The dominant failure mode of that speed is **re-introducing bugs**: a fix reveals the next bug, an invariant written only in prose gets re-violated, a green test was lying, a thing that "works" works only on the machine it was built on.

The root cause is that safely changing a piece of code ("a **sector**" — a module or service) requires knowing **seven dimensions** of it, but only the first is fully visible in the code; the rest live invisibly in people's heads, in systemd, in prose docs, in a bug catalog nobody opens, and in test suites no one mapped to what they actually cover. **Sentinel** surfaces all seven on one graph (the **Map**), turns the invisible ones into loud automatic checks (the **Gates**), and wires both into every change (the **Loop**) — kept current in real time by a background watcher, and served to the human as a **live dashboard** and to the agent as a **CLI**.
...
| # | Dimension | Where it lives today | A real Klik bug from *not* seeing it |
|---|---|---|---|
| 1 | **Structure** — responsibilities, schema, file tree, dependencies | the code (parseable) | — (the visible half) |
| 2 | **Behavior** — invariants, real runtime data/edge-cases, concurrency, failure modes | runtime + people's heads | `encodeDefaults` dropped a field → 403 |
| 3 | **Context** — env/config, deploy-state-vs-git, resource ownership, infra/routing | systemd, nginx, the server | `subscription_api` bare-env crash-loop |
| 4 | **Boundaries** — forks/parallel impls that must sync, external systems | tribal knowledge | the two-fork drift; the unpowered mic rail |
| 5 | **Intent & History** — the *why*, past bugs + why fixes failed, constraints (BIPA, COPPA, isolation) | ADRs, git, a catalog | re-introduced already-fixed bugs |
| 6 | **Change-safety** — true blast radius, observability (how you'd know it broke in prod) | nowhere | the missing/lying feedback loop |
| 7 | **Tests & Coverage** — does this sector have unit tests, what they cover, pass/fail, **which tests cover a change (own + consumers')**, and the **uncovered surface** you're about to touch | test files + coverage reports + CI results (rarely mapped to sectors) | a change to an **untested path** shipped silently (the ~5%-coverage pipeline areas) |

**Only dimension #1 is fully parseable from code.** Off-the-shelf code-graph tools (graphify, GitNexus, Understand-Anything) draw #1 and stop — so the regressions keep coming from #2–#7, which no parser surfaces. Notably, **none of them maps tests to what they cover or shows the uncovered surface** (#7) — the single most important thing for *preventing* a regression. "It works" too often means "works on the one tree / machine / process I'm holding," not "reproducible from a clean checkout, with the covering tests green."

This is not a tooling gap alone; it is the reason the team keeps paying the same bug tax. Sentinel exists to close it.
...
2. **The Gates** — guards / CI / the deploy-parity gate / the bug catalog / import-linter that make the invisible dimensions **fail loud and automatically**. *Mostly built already.*
...
- **US1 (blast radius)** — *As a dev*, before I edit `KK_auth`, I see the services + client call-sites that depend on it **and the tests that cover them**, so I run those first.
...
- **US3 (intent & history)** — *As a dev*, I see this sector previously had the `encodeDefaults` 403 bug, with the guard that now prevents it, so I don't re-open it.
...
- **FR-M2** — Parse **Kotlin (KMP, incl. Compose) + Python (FastAPI)**: files, imports, classes/functions, `@Serializable`/Pydantic **schema**, and **tests**.
...
- **FR-M5 — Schema extraction** from `@Serializable` / Pydantic / SQL DDL.
...
- **FR-M10 — Span two repos** (KMP frontend + Python backend) in one map.
- **FR-M11 — Tests & Coverage dimension (MUST)**: for every sector, surface (a) its unit-test inventory, (b) **coverage %** and pass/fail (ingested from coverage reports — Kover/coverage.py — and the latest CI/test run), (c) the **test→target mapping** (`TESTS`/`COVERS` edges), and (d) the **uncovered surface** of a pending change (the symbols a change touches that no test exercises). Available on both the CLI and the live dashboard; the dashboard MUST render a per-sector **Tests panel** (coverage, pass/fail, covering tests own + consumers', uncovered badge).

### The Gates (FR-G) — *largely exist; integrate them*
- **FR-G1** — Every known bug-class has a loud automatic check (the `bug-regression-catalog` lints; `k1-guards`).
- **FR-G2** — Deploy-parity gate: running stack == committed model (`klik-deploy-parity`, every 5 min) — feeds the dashboard's Context overlay.
- **FR-G3** — Service-contract enforcement: `import-linter` for FastAPI service layers (fail CI on violation).
...
- **NFR1 — Languages**: Kotlin Multiplatform + Python FastAPI, both first-class.
...
- **NFR5 — Trustworthy accuracy**: blast-radius and coverage must not **silently** under-report; where Kotlin edges are heuristic (no name-resolution in v1) or coverage is stale, the map must **say so** rather than imply completeness.
- **NFR6 — Low friction**: must not slow the change loop; the watcher auto-refreshes, no manual rebuild step.
- **NFR7 — Maintainable foundation**: no archived/abandoned core dependencies (→ **Neo4j**, not the archived Kùzu).
...
**In — v1 (the backbone):** structural map (Kotlin + Python → Neo4j) · the **watcher daemon** (real-time freshness, incl. coverage/CI ingest) · blast-radius + consumers'-tests + schema + **coverage** queries · the **Tests & Coverage dimension (MUST)** · the **CLI** · the **LIVE dashboard (MUST)** with push + the **Tests panel** · one sector end-to-end → then repo-wide · wire the existing gates (parity / catalog / k1-guards) as the Context & Behavior overlays.
...
**Out:** any zero-bug guarantee · replacing CI / tests / review · general code search or RAG · multimodal docs · languages beyond KMP-Kotlin + Python · an MCP server (deferred indefinitely; CLI is the decided agent surface).

---

## 11. Constraints, Dependencies & Risks

- **Build the glue, adopt the hard parts.** Backbone + CLI ≈ **8–13 focused build-days**; the **LIVE dashboard adds ~3–5 days** (React Flow + a small websocket/SSE server); the **Tests & Coverage dimension adds ~2–3 days** (parse coverage reports — Kover XML / `coverage.py` — + map tests→targets + ingest CI results). Parse with `tree-sitter-kotlin` (structure only — **no name resolution**) + Python `ast`; optionally ingest **SCIP**/`CodeQL` for precise cross-symbol edges. Store in **Neo4j** (maintained) — **not Kùzu (archived 2025-10)**.
...
- **Coverage source**: dimension #7 depends on the repos emitting coverage (Kover for KMP, `coverage.py`/pytest-cov for Python) and CI publishing test results; where absent, the sector shows "coverage unknown" (honest, per NFR5) — and that's itself a useful signal.
- **GitNexus** is the closest off-the-shelf fit but is **PolyForm Noncommercial** → **blocked** for commercial Klik. **graphify** (MIT) is viable for a quick structural v0.
- **Two repos**: the Python backend (`minervacap2022/Klik` / `ssh gcp:/opt/Klik`) is not in the frontend checkout — the map must index both.
- **Risk — Kotlin precision**: `scip-kotlin` pins to one Kotlin major + KMP source-set friction; v1 Kotlin edges are import-level + heuristic (acceptable for ~80% of blast-radius, but must be labeled — NFR5).
...
- Memory `project_code_graph_sector_map.md` — verified code-graph tool research (graphify / GitNexus / Understand-Anything verdicts + sources).
- KLIK 开发规范 wiki — 规划中 **011** (防回归与变更安全) and **012** (实时 Sector 地图).
